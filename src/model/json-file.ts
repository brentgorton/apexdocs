...
export class JSONFile extends OutputFile {
  public setContents(title: string, contents: Type[]) {
    this._contents = JSON.stringify({ className: title, contents }, null, 2);
  }
  ...
  private _replaceInlineReferences(text: string): string {
    text = JSONFile.replaceInlineLinks(text);
    text = JSONFile.replaceInlineEmails(text);
    return text;
  }
}
