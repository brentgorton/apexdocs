...
import { declareType, declareMethod, declareField } from './json-generation-util';
...
export class JSONTypeFile extends JSONFile implements WalkerListener {
  ...
  private addInnerTypes(title: string, types: Type[], addSeparator = true) {
    ...
    types
      ...
      .forEach((currentType) => {
        const innerFile = new JSONTypeFile(currentType, this.headingLevel + 2, undefined, true);
